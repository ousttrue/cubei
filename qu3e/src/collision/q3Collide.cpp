...
// qBoxtoBox
//--------------------------------------------------------------------------------------------------
inline bool q3TrackFaceAxis( int* axis, int n, float s, float* sMax, const q3Vec3& normal, q3Vec3* axisNormal )
{
	if ( s > float( 0.0 ) )
		return true;
...
//--------------------------------------------------------------------------------------------------
inline bool q3TrackEdgeAxis( int* axis, int n, float s, float* sMax, const q3Vec3& normal, q3Vec3* axisNormal )
{
	if ( s > float( 0.0 ) )
		return true;
...
//--------------------------------------------------------------------------------------------------
void q3ComputeReferenceEdgesAndBasis( const q3Vec3& eR, const q3Transform& rtx, q3Vec3 n, int axis, uint8_t* out, q3Mat3* basis, q3Vec3* e )
{
...
int q3Clip( const q3Vec3& rPos, const q3Vec3& e, uint8_t* clipEdges, const q3Mat3& basis, q3ClipVertex* incident, q3ClipVertex* outVerts, float* outDepths )
{
	int inCount = 4;
	int outCount;
	q3ClipVertex in[ 8 ];
...
void q3BoxtoBox( q3Manifold* m, q3Box* a, q3Box* b )
{
	...
	if ( !parallel )
	{
		...
		if ( q3TrackEdgeAxis( &eAxis, 6, s, &eMax, q3Vec3( float( 0.0 ), -C[ 0 ][ 2 ], C[ 0 ][ 1 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 7, s, &eMax, q3Vec3( float( 0.0 ), -C[ 1 ][ 2 ], C[ 1 ][ 1 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 8, s, &eMax, q3Vec3( float( 0.0 ), -C[ 2 ][ 2 ], C[ 2 ][ 1 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 9, s, &eMax, q3Vec3( C[ 0 ][ 2 ], float( 0.0 ), -C[ 0 ][ 0 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 10, s, &eMax, q3Vec3( C[ 1 ][ 2 ], float( 0.0 ), -C[ 1 ][ 0 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 11, s, &eMax, q3Vec3( C[ 2 ][ 2 ], float( 0.0 ), -C[ 2 ][ 0 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 12, s, &eMax, q3Vec3( -C[ 0 ][ 1 ], C[ 0 ][ 0 ], float( 0.0 ) ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 13, s, &eMax, q3Vec3( -C[ 1 ][ 1 ], C[ 1 ][ 0 ], float( 0.0 ) ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 14, s, &eMax, q3Vec3( -C[ 2 ][ 1 ], C[ 2 ][ 0 ], float( 0.0 ) ), &nE ) )
			return;
	}
