...
void epxDetectCollision(const EpxState *states,
  ...
  for (EpxUInt32 i = 0; i < pairs.size(); i++) {
    ...
    for (EpxUInt32 j = 0; j < collA.m_numShapes; j++) {
      ...
      EpxTransform3 offsetTransformA(shapeA.m_offsetOrientation,
                                     shapeA.m_offsetPosition);
      ...
      for (EpxUInt32 k = 0; k < collB.m_numShapes; k++) {
        ...
        EpxTransform3 offsetTransformB(shapeB.m_offsetOrientation,
                                       shapeB.m_offsetPosition);
        ...
        if (epxConvexConvexContact(shapeA.m_geometry, worldTransformA,
                                   shapeB.m_geometry, worldTransformB, normal,
                                   penetrationDepth, contactPointA,
                                   contactPointB) &&
          ...
          pair.contact->addContact(
              ...
              offsetTransformA.getTranslation() +
                  offsetTransformA.getUpper3x3() * contactPointA,
              offsetTransformB.getTranslation() +
                  offsetTransformB.getUpper3x3() * contactPointB);
        }
      }
    }
  }
