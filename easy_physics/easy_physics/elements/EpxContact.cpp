...
int EpxContact::findNearestContactPoint(const EpxVector3 &newPointA,const EpxVector3 &newPointB,const EpxVector3 &newNormal)
{
	...
	for(EpxUInt32 i=0;i<m_numContacts;i++) {
...
{
	m_numContacts = 0;
	for(int i=0;i<EPX_NUM_CONTACTS;i++) {
...
{
	for(EpxUInt32 i=0;i<contact.m_numContacts;i++) {
		...
		else if(m_numContacts < EPX_NUM_CONTACTS) {
			// 衝突点を新規追加
	...
	const EpxVector3 &contactPointB)
{
	...
	if(id < 0 && m_numContacts < EPX_NUM_CONTACTS) {
		// 衝突点を新規追加
		id = m_numContacts++;
