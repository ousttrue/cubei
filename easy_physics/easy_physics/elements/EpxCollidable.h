...
struct EpxCollidable {
  ...
  /// 空きがなければ無視される
  /// @param shape 形状
  EpxShape *addShape(const EpxShape &shape) {
    if (m_numShapes < EPX_NUM_SHAPES) {
      m_shapes[m_numShapes] = shape;
      auto added = &m_shapes[m_numShapes];
      ++m_numShapes;
      return added;
    } else {
      return nullptr;
  ...
  void finish() {
    ...
    for (EpxUInt32 i = 0; i < m_numShapes; i++) {
      ...
      for (EpxUInt32 v = 0; v < mesh.m_numVertices; v++) {
        aabbMax =
            maxPerElem(aabbMax, m_shapes[i].m_offsetPosition +
                                    rotate(m_shapes[i].m_offsetOrientation,
                                           mesh.m_vertices[v]));
        aabbMin =
            minPerElem(aabbMin, m_shapes[i].m_offsetPosition +
                                    rotate(m_shapes[i].m_offsetOrientation,
                                           mesh.m_vertices[v]));
      }
    }
