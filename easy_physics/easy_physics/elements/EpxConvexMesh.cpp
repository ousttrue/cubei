...
void epxGetProjection(EpxFloat &pmin, EpxFloat &pmax,
                      const EpxConvexMesh *convexMesh, const EpxVector3 &axis) {
  assert(convexMesh);
...
EpxBool epxCreateConvexMesh(EpxConvexMesh *convexMesh, EpxShapeType shapeType,
  ...
  if (numVertices > EPX_CONVEX_MESH_MAX_VERTICES ||
  ...
  for (EpxUInt32 i = 0; i < numIndices / 3; i++) {
    EpxVector3 p[3] = {convexMesh->m_vertices[indices[i * 3]],
  ...
  // エッジバッファ作成
  EpxUInt8 edgeIdTable[EPX_CONVEX_MESH_MAX_VERTICES *
                       EPX_CONVEX_MESH_MAX_VERTICES / 2];
  ...
  for (EpxUInt32 i = 0; i < convexMesh->m_numFacets; i++) {
    ...
    for (EpxUInt32 e = 0; e < 3; e++) {
      EpxUInt32 vertId0 =
          EPX_MIN(facet.vertId[e % 3], facet.vertId[(e + 1) % 3]);
      EpxUInt32 vertId1 =
          EPX_MAX(facet.vertId[e % 3], facet.vertId[(e + 1) % 3]);
      ...
      if (edgeIdTable[tableId] == 0xff) {
        ...
        assert(edgeIdTable[tableId] < EPX_CONVEX_MESH_MAX_EDGES);
