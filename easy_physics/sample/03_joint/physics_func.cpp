...
EpxBool broadPhaseCallback(EpxUInt32 rigidBodyIdA, EpxUInt32 rigidBodyIdB,
                           void *userData) {
...
void physicsSimulate() {
  ...
  perf.begin();
  g_numPairs[g_pairSwap] = epxBroadPhase(
      {states, g_numRigidBodies}, {collidables, g_numRigidBodies},
      {pairs[1 - g_pairSwap], g_numPairs[1 - g_pairSwap]},
      {pairs[g_pairSwap], maxPairs}, &allocator, NULL, broadPhaseCallback);
  perf.end("broadphase");
...
static void createSceneBallJoint(Geometry &scene) {
  // 地面
  {
  ...
  EpxBallJoint &joint = joints[g_numJoints++];
  joint.reset();
...
static void createSceneHingeJoint(Geometry &scene) {
...
static void createSceneFixedJoint(Geometry &scene) {
  // ボールジョイントx3で固定ジョイントを再現
  EpxBallJoint &joint0 = joints[g_numJoints++];
  EpxBallJoint &joint1 = joints[g_numJoints++];
  EpxBallJoint &joint2 = joints[g_numJoints++];
  joint0.reset();
  joint1.reset();
  joint2.reset();
...
static int createGear(Geometry &scene, const EpxVector3 &offsetPosition,
...
static void createSceneGearJoint(Geometry &scene) {
  // ギア大
...
static void createSceneChain(Geometry &scene) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    states[id].reset();
    states[id].m_position = EpxVector3(-2.5f, 5.0f, 0.0f) +
    ...
    if (i > 0) {
      ...
      joint.anchorA =
          states[id].m_position + EpxVector3(0.0f, chainScale[1], 0.0f);
    }
    joint.rigidBodyB = id;
...
static const char titles[][32] = {
    "ball joint", "hinge joint", "fixed joint", "gear joint", "chain joint",
};
