...
void castRays()
{
	...
	if(doAreaRaycast) {
		...
		param.proxiesYb = proxies[4];
		param.proxiesZb = proxies[5];
		param.numProxies = numProxiesInWorld;
...
void createSceneLandscape()
{
	...
	bodies[id].reset();
	states[id].reset();
	states[id].setPosition(PfxVector3(0.0f,-1.5f,0.0f));
	states[id].setOrientation(PfxQuat::rotationY(0.15f));
	states[id].setMotionType(kPfxMotionTypeFixed);
	states[id].setRigidBodyId(id);
}
...
void physicsCreateScene(int sceneId)
{
	...
	memset(rayInputs,0,sizeof(PfxRayInput)*NUM_RAYS);
	memset(rayOutputs,0,sizeof(PfxRayOutput)*NUM_RAYS);
	numRays = 0;
