...
inline 
PfxUInt32 PfxCollidable::getMaxNumShapes() const
{
	return m_maxShapes;
}
...
{
	SCE_PFX_ALWAYS_ASSERT(i>0&&i<m_numShapes);
...
{
	SCE_PFX_ALWAYS_ASSERT(i<m_numShapes);
	if(i>0) {
...
{
	SCE_PFX_ALWAYS_ASSERT(m_numShapes<=m_maxShapes);
	if(m_numShapes == 0) {
		m_numShapes++;
		return m_defShape;
	}
