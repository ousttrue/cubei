...
{
#if defined(_WIN32) && defined(SCE_PFX_USE_SIMD_VECTORMATH)
#if defined(SCE_PFX_USE_FREE_VECTORMATH)
	__m128 x = src.get128();
#else
	__m128 x = src.get128().f128;
#endif
	__m128 y = _mm_shuffle_ps(x, x, _MM_SHUFFLE(((1)+3)&3, ((1)+2)&3, ((1)+1)&3, ((1)+0)&3));
	__m128 z = _mm_shuffle_ps(x, x, _MM_SHUFFLE(((2)+3)&3, ((2)+2)&3, ((2)+1)&3, ((2)+0)&3));
	_mm_store_ss(&fptr[0], x);
	_mm_store_ss(&fptr[1], y);
	_mm_store_ss(&fptr[2], z);
#else
	storeXYZ(src, fptr);
#endif
}
...
void pfxGetRotationAngleAndAxis(const PfxQuat &unitQuat,PfxFloat &angle,PfxVector3 &axis)
{
	...
	if(pfxAbsf(unitQuat.getW()) < 1.0f-epsilon && lengthSqr(unitQuat.getXYZ()) > epsilon) {
		PfxFloat angleHalf = acosf(unitQuat.getW());
		PfxFloat sinAngleHalf = sinf(angleHalf);

		if(pfxAbsf(sinAngleHalf) > 1.0e-10f) {
