...
{
	return 16 + SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphaseProxy) * numRigidBodies);
}

PfxUInt32 pfxGetWorkBytesOfUpdateBroadphaseProxies(PfxUInt32 numRigidBodies,PfxUInt32 maxTasks)
{
	(void)maxTasks;
	return 16 + SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphaseProxy) * numRigidBodies) * 6;
}

PfxUInt32 pfxGetWorkBytesOfFindPairs(PfxUInt32 maxPairs,PfxUInt32 maxTasks)
{
	return 16 + SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphasePair) * maxPairs) * maxTasks;
}

PfxUInt32 pfxGetPairBytesOfFindPairs(PfxUInt32 maxPairs)
{
	return SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphasePair) * maxPairs);
}

PfxUInt32 pfxGetWorkBytesOfDecomposePairs(PfxUInt32 numPreviousPairs,PfxUInt32 numCurrentPairs,int maxTasks)
{
	return 16 +
		SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphasePair)*numPreviousPairs) + 
		SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphasePair)*numPreviousPairs) * maxTasks + 
		SCE_PFX_ALLOC_BYTES_ALIGN16(sizeof(PfxBroadphasePair)*numCurrentPairs) * maxTasks;
}
...
PfxInt32 pfxCheckParamOfUpdateBroadphaseProxies(const PfxUpdateBroadphaseProxiesParam &param)
{
	...
	if(param.outOfWorldBehavior & (PfxUInt32)0xfffffffc) return SCE_PFX_ERR_INVALID_VALUE;
...
PfxInt32 pfxUpdateBroadphaseProxies(PfxUpdateBroadphaseProxiesParam &param,PfxUpdateBroadphaseProxiesResult &result)
{
	...
	for(int i=0;i<(int)param.numRigidBodies;i++) {
		...
		else if(chk == SCE_PFX_ERR_OUT_OF_WORLD) {
	...
	PfxBroadphaseProxy *workProxies = (PfxBroadphaseProxy*)pool.allocate(sizeof(PfxBroadphaseProxy)*param.numRigidBodies);
	...
	for(PfxUInt32 i=0;i<numProxies;i++) {
		for(PfxUInt32 j=i+1;j<numProxies;j++) {
			...
			if(	pfxCheckCollidableInBroadphase(proxyA,proxyB) ) {
				if(numPairs >= maxPairs) return SCE_PFX_ERR_OUT_OF_MAX_PAIRS;
