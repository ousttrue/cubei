				...
				float contactThreshold)
{
	...
	if(shapeA.getType() == kPfxShapeCylinder) {
		...
		if(shapeB.getType() == kPfxShapeSphere) {
			...
			err = gjk.collide(d,nml,pA,pB,worldTransformA,worldTransformB,SCE_PFX_FLT_MAX);
		}
	...
	if(err == kPfxGjkResultOk && d < contactThreshold) {
		contacts.addContactPoint(
			d,
			worldTransformA.getUpper3x3()*nml,
			offsetTransformA*pA,
			offsetTransformB*orthoInverse(worldTransformB)*worldTransformA*pB,
			PfxSubData());
	}
}
				...
				float contactThreshold)
{
	...
	if (lmeshA->isUsingBvh())
	{
		pfxContactLargeTriMeshBvh(localContacts, lmeshA, worldTransformA, shapeB, worldTransformB, contactThreshold);
	}
	else
	{
		pfxContactLargeTriMesh(localContacts, lmeshA,worldTransformA, shapeB, worldTransformB, contactThreshold);
	}
	...
	const PfxLargeTriMesh *lmeshB = shapeB.getLargeTriMesh();
		...
		PfxContactCache localContacts;
	if (lmeshB->isUsingBvh())
	{
		pfxContactLargeTriMeshBvh(localContacts, lmeshB, worldTransformB, shapeA,worldTransformA, contactThreshold);
	}
	else
	{
		pfxContactLargeTriMesh(localContacts, lmeshB, worldTransformB, shapeA,worldTransformA, contactThreshold);
	}
