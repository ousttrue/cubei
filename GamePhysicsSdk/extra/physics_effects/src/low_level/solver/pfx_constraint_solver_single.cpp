...
{
	if(!param.workBuff) return SCE_PFX_ERR_INVALID_VALUE;
...
PfxInt32 pfxCheckParamOfApplyDeltaVelocity(const PfxApplyDeltaVelocityParam &param)
{
	if(!param.states || !param.solverBodies ) return SCE_PFX_ERR_INVALID_VALUE;
	if(!SCE_PFX_PTR_IS_ALIGNED16(param.states) || !SCE_PFX_PTR_IS_ALIGNED16(param.solverBodies)) return SCE_PFX_ERR_INVALID_ALIGN;
	return SCE_PFX_OK;
}
...
{
	PfxInt32 ret = pfxCheckParamOfApplyDeltaVelocity(param);
	if(ret != SCE_PFX_OK) return ret;
	
	SCE_PFX_PUSH_MARKER("pfxApplyDeltaVelocity");
