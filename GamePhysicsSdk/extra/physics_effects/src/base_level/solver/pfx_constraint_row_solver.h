...
void pfxCalcJointAngleSwing1Swing2Twist(PfxMatrix3 &worldFrameA,PfxMatrix3 &worldFrameB,PfxFloat *angle,PfxVector3 *axis)
{
	...
	if(pfxAbsf(frameBA.getCol0()[1]) < pfxAbsf(frameBA.getCol0()[2])) {
...
void pfxCalcJointAngleUniversal(PfxMatrix3 &worldFrameA,PfxMatrix3 &worldFrameB,PfxFloat *angle,PfxVector3 *axis)
{
	...
	axis[2] = worldFrameA * axis[2];
#else
	PfxVector3 rot = PfxQuat::euler(PfxQuat(frameBA),(sce::Vectormath::Scalar::RotationOrder)3);
	angle[0] = rot[0];
	angle[1] = rot[2];
	angle[2] = rot[1];
	axis[0] = worldFrameA.getCol0(); // X
	axis[1] = worldFrameA.getCol2(); // Z
	axis[2] = worldFrameA.getCol1(); // Y
#endif
}
