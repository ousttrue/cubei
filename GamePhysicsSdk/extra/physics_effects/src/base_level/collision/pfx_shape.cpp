...
void pfxGetShapeAabbLargeTriMesh(const PfxShape &shape,PfxVector3 &aabbMin,PfxVector3 &aabbMax)
{
	...
	PfxVector3 half = absPerElem(rot) *  largemesh->m_half;
	aabbMin = rot * largemesh->m_offset + shape.getOffsetPosition() - half;
	aabbMax = rot * largemesh->m_offset + shape.getOffsetPosition() + half;
}
